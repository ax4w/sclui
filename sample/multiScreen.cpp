...
int main(void) {
    ...
    Button *b1 = new Button("quit",2,4,COLOR_BLUE,COLOR_RED);
    b1->onButtonPress = &doQuit;
    subScreen1->addItem(b1);
    mainScreen.addSubscreen(subScreen1);

    Screen *subScreen2 = new Screen("S2", 30, 10, '-','|' , 12, 12);
    Button *b2 = new Button("next",2,4,COLOR_BLUE,COLOR_RED);
    TextBox *text = new TextBox("textbox",2,6,10,COLOR_WHITE,COLOR_GREEN, &TextBoxFilterAllowAll,'|');

    b2->onButtonPress = &switchScreen;
    subScreen2->addItem(b2);
